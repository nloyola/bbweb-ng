...
export class StudyService {
  ...
  get(slug: string): Observable<Study> {
    return this.http.get<ApiReply>(`${this.BASE_URL}/${slug}`).pipe(map(this.replyToStudy));
  }
    ...
    return this.http.get<ApiReply>(`${this.BASE_URL}/search`, { params }).pipe(
      ...
      map((reply: ApiReply) => {
        ...
        if (reply && reply.data && jObj.items) {
          const entities: Study[] = (jObj.items as JSONArray).map((obj: JSONObject) =>
            new Study().deserialize(obj as any)
          );
          return {
            searchParams,
            entities,
  ...
  searchCollectionStudies(searchParams: SearchParams): Observable<StudyStateInfo[]> {
    let params = searchParamsToHttpParams(searchParams);
    return this.http.get<ApiReply>(`${this.BASE_URL}/collectionStudies`, { params }).pipe(
      // delay(10000),
      map((reply: ApiReply) => {
        if (reply && reply.data) {
          return (reply.data as JSONArray).map((obj: JSONObject) =>
            new StudyStateInfo().deserialize(obj as any)
          );
        }
        throw new Error('expected a paged reply');
      })
  ...
  removeAnnotationType(study: Study, annotationTypeId: string): Observable<Study> {
    ...
    return this.http.delete<ApiReply>(url).pipe(map(this.replyToStudy));
  }

  enableAllowed(studyId: string): Observable<any> {
    return this.http.get<ApiReply>(`${this.BASE_URL}/enableAllowed/${studyId}`).pipe(
      // delay(1000),
      map((reply: ApiReply) => {
