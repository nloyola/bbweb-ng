...
export class SpecimenService {
  ...
  get(slug: string): Observable<Specimen> {
    return this.http.get<ApiReply>(`${this.BASE_URL}/${slug}`).pipe(map(this.replyToSpecimen));
  }
  ...
  search(event: CollectionEvent, searchParams: SearchParams): Observable<PagedReply<Specimen>> {
    ...
    return this.http.get<ApiReply>(`${this.BASE_URL}/${event.slug}`, { params }).pipe(
      // delay(1000),
      map((reply: ApiReply) => {
        const jObj = reply.data as JSONObject;
        if (reply && reply.data && jObj.items) {
          const entities: Specimen[] = (jObj.items as JSONArray).map((obj: JSONObject) =>
            new Specimen().deserialize(obj as any)
          );
