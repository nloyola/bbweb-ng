...
export class StudyStoreEffects {
  constructor(
    private actions$: Actions<StudyActions.StudyActionsUnion>,
  ...
  addRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(action =>
      this.studyService.add(action.study).pipe(
  ...
  updateRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(action =>
      this.studyService.update(action.study, action.attributeName, action.value).pipe(
