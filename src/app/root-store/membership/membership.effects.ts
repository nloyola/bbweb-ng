...
export class MembershipStoreEffects {
  constructor(
    private actions$: Actions<MembershipActions.MembershipActionsUnion>,
  ...
  getRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(action =>
      this.membershipService.get(action.slug).pipe(
  ...
  searchRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(action =>
      this.membershipService.search(action.searchParams).pipe(
  ...
  addRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(action =>
      this.membershipService.add(action.membership).pipe(
  ...
  updateRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(action =>
      this.membershipService.update(action.membership, action.attributeName, action.value).pipe(
  ...
  @Effect()
  removeMembershipRequest$: Observable<Action> = this.actions$.pipe(
    ofType(MembershipActions.removeMembershipRequest.type),
    switchMap(action =>
      this.membershipService.removeMembership(action.membership).pipe(
        // delay(2000),
