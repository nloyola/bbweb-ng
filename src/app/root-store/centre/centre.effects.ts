...
export class CentreStoreEffects {
  constructor(
    private actions$: Actions<CentreActions.CentreActionsUnion>,
  ...
  getRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(props =>
      this.centreService.get(props.slug).pipe(
  ...
  searchRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(props =>
      this.centreService.search(props.searchParams).pipe(
  ...
  @Effect()
  searchLocationsRequest$: Observable<Action> = this.actions$.pipe(
    ofType(CentreActions.searchLocationsRequest.type),
    switchMap(props =>
      this.centreService.searchLocations(props.filter).pipe(
        map(searchReply => CentreActions.searchLocationsSuccess({ searchReply })),
        catchError(error => observableOf(CentreActions.searchLocationsFailure({ error })))
      )
    )
  );
  ...
  addRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(props =>
      this.centreService.add(props.centre).pipe(
  ...
  updateRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(props =>
      this.centreService.update(props.centre, props.attributeName, props.value).pipe(
  ...
  countsRequest$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(() =>
      this.centreService.counts().pipe(
        // delay(5000),
