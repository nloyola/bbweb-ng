...
export const selectSpinnerEntity = createFeatureSelector<fromSpinner.State>('spinner');

export const isActive = (state: fromSpinner.State): boolean => state.active > 0;

export const selectSpinnerIsActive: MemoizedSelector<object, boolean> = createSelector(
  selectSpinnerEntity,
  isActive
);
